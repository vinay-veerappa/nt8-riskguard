...
## Implementer notes
- EngineFields: added `_accountsIgnoringChange` (HashSet<string>, guarded by `_lock`) to remember accounts whose provider has demonstrably ignored `Account.Change()`.
- FollowerBracket: added per-leg requested-change fields (`StopChangeRequested`/`StopChangeRequestedPrice`/`StopChangeRequestedQuantity` and the target equivalents) so the settle event can compare what was asked against what the broker actually holds.
- SyncFollowerStopOnce: records the requested price/quantity under `_lock` after `Change()` returns without throwing; checks `_accountsIgnoringChange` before calling `Change()` and bypasses straight to cancel-then-create once the account is marked; clears stale request flags whenever the leg is replaced.
- SyncFollowerTargetOnce: same treatment as the stop leg, independently, so target verification/ fallback never delays or disturbs the risk leg.
- ResetBracketsForTest: now clears `_accountsIgnoringChange` alongside `_followerBrackets` so the session-scoped account verdict does not leak between tests.
- OnFollowerOrderUpdate: added synchronous read-back verification inside the existing `AcceptsModification` block, before the `OccupiesSlot` early return and before `ReDriveDeferredLeg`; on mismatch it logs `BRACKET_CHANGE_IGNORED`, marks the account as ignoring `Change()`, clears the deferred flag for that leg, and re-drives through the existing cancel-then-create path (`SyncFollowerStopOnce`/`SyncFollowerTargetOnce`).
- No new config keys. The account-level bypass is the only new runtime state; it is intentionally session-scoped in production and explicitly reset in tests.


## REGION "EngineFields" (addons/TradeCopierEngine.cs)
### BEFORE
```csharp
...
## REGION "FollowerBracket" (addons/TradeCopierEngine.cs)
### BEFORE
```csharp
...
## REGION "SyncFollowerStopOnce" (addons/TradeCopierEngine.cs)
### BEFORE
```csharp
...
## REGION "SyncFollowerTargetOnce" (addons/TradeCopierEngine.cs)
### BEFORE
```csharp
...
## REGION "ResetBracketsForTest" (addons/TradeCopierEngine.cs)
### BEFORE
```csharp
...
## REGION "OnFollowerOrderUpdate" (addons/TradeCopierEngine.cs)
### BEFORE
```csharp
