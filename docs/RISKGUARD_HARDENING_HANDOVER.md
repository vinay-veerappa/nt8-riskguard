...
> ### Do next (session 56): `P1-139`, then `P1-140`, then `P2-127`. Both new items are `P1` and both are in `DynamicAtmManager.cs`, found by reading `P2-136`'s neighbourhood rather than by a report — so they outrank the UI work below on band, not on novelty. `P1-139`: one refused trailing move makes the breakeven branch ask the broker to put the stop BACK at breakeven, because `BreakevenTriggered` means three different things and ✅ CLOSED `P0-67` gave it a fourth. Reproduced deterministically — the stop is asked down from 20010.00 to 20000.50 on a live long, in the SAME sweep as the refusal. ⚠️ **The direction guard alone is not the fix**: it leaves the flag false and the trailing block is gated on it, which restores ✅ CLOSED `P0-67`'s original defect. `P1-140`: the partial-profit order is submitted into the stop and target's own OCO group while both of those are sized for the FULL quantity — every outcome NT8 can pick is a defect (naked remainder / flipped position / silently dead feature), and `Math.Floor(1 * 0.50) == 0` is why no 1-lot test or live bracket has ever reached the block. ⚠️ **Neither is live-validated and neither can be on `Sim101` alone**; `P1-139` needs a provider declining a stop move, `P1-140` needs 2+ contracts.
>
