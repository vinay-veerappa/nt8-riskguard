...
### P2-132. In `shadow`, the rule inventory cannot distinguish a rule that JUST FIRED from one that has never fired — measured on the funded account while `MAX_SIZE_BREACH` was live — ✅ CLOSED 2026-08-21 (session 63): **slice (a) DONE + DEPLOYED `v1.62.0`** — the 'Max contracts per account' rule now reports `currentValue` from the account's max single-instrument position, derived from `state.Positions.Values` (the SAME source the `MAX_SIZE_BREACH` enforcer iterates in `EvaluateRules`, `if (pos.Quantity > limit)`), so a rule in breach no longer renders null while three neighbours report where they stand. **Slice (b) DONE 2026-08-21 (session 63)**: the aggregate cap now reports the cross-account SUM of non-flat quantity (`TotalPositionQuantity` summed over `AllAccounts`, the same number `EvaluateAggregateSizing` sums into `totalAggregateContracts`), and both sizing rules carry a `Breached` flag folded out of the enforcer's own `> limit` comparison plus a `LastFiredUtc` timestamp recorded by `MarkRuleLockout` (the ONE funnel every lockout-capable rule passes through) and copied into the snapshot — so "never fired" and "fired a minute ago" are different answers. ⚠️ This slice was driven through the agent loop, which produced a green candidate that read `account.Positions` (live) instead — a plausible-sounding but WRONG source (the enforcer reads cached `state.Positions`); caught by hand-arbitration after the panel returned `NOT_CONVERGING`. Found 2026-08-16 (session 52). Slice (a) DEPLOYED live `v1.62.0` 2026-08-21 (session 59). ✅ **`mutate_p2132.py` (9 mutants, 9/9 killed) now covers BOTH slices** — including the population paths (`GetAccountSnapshots`, `MarkRuleLockout`) that the hand-built-snapshot tests could not see, which the battery's first run exposed as three survivors. Suite 3524/0. ✅ **DEPLOYED `v1.64.0`** (after the slice-(b) corrections below; the box previously ran `v1.62.0`).

> **⚠️ Slice (b) CORRECTIONS — critical review 2026-08-21 (session 63), fixed same session (`v1.63.0` dev).** The first slice-(b) commit (`dbd8013`) shipped RED and its headline claim — "the inventory cannot disagree with the enforcer" — did not hold. Four defects, all fixed:
> 1. **CI red — `RuleLastFired` unclassified.** A new `AccountState` field decided runtime-only in a *code comment* but never added to `check_account_state_persisted.py`'s `RUNTIME_ONLY` table, so the P1-170 gate failed. The commit message read "Suite 3524/0" — the unit suite passed while a source gate was red. Now classified in the gate.
> 2. **Aggregate `LastFiredUtc` was a green that can never be red.** `AGGREGATE_SIZE_BREACH` flattens but does **not** lock out, so it never funnels through `MarkRuleLockout` — the only writer of `RuleLastFired`. The aggregate row read "never fired" no matter how often it fired. Fixed by extracting `RecordRuleFired` (recency without the lockout side-effects) and calling it from `EvaluateAggregateSizing` on every account the breach spans. The "one funnel" claim above is true only for *lockout-capable* rules.
> 3. **Aggregate breach disagreed with the enforcer under `ExpectedCopies > 1`.** The display compared the gross cross-account SUM, but the enforcer compares `normalizedAggregate` (= max single-account position when copies > 1). In the copier scenario the product exists for, the sum flags a breach the enforcer never acts on. Now both value and flag come from `AggregateNormalizedQuantity`, the enforcer's own number (identical to the sum when copies ≤ 1).
> 4. **Per-account breach ignored the resolved per-instrument limit.** The display compared `MaxPositionQuantity > Sizing.MaxContractsPerAccount` (the flat config cap), but the enforcer compares each position to `ResolveMaxContracts(...)` — which folds in `InstrumentLimits` (the funded account's `MNQ:1`, P1-159). A 3-lot MNQ read green while the enforcer flattened it. Now `GetAccountSnapshots` computes the breach the enforcer's way into `MaxPositionQuantityBreached`, and the evaluator reads that flag.
>
> `mutate_p2132.py` grew from 9→13 mutants (copier-normalization, population-path breach, resolved-limit, aggregate-firing), **13/13 killed**. Suite **3531/0**. Tests added: `AggregateCapUsesNormalizedNotSumUnderCopies`, `AnAggregateBreachRecordsTheFiring`, `GetAccountSnapshotsFlagsAPerAccountBreach`. ✅ **DEPLOYED live `v1.64.0` 2026-08-21 (session 63)** — synced + hot-swap compiled (0 errors) while all 104 accounts were flat and the guard in `shadow`; `nt_health` reads `version 1.64.0`, `isArmed true`, `guarding true`.
